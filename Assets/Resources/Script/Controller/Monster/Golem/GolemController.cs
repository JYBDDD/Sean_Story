...
public class GolemController : MonsterBaseController
{
    ...
    {
        _stat = JsonConvert.DeserializeObject<StatData>(GameManager.Resource.Load<TextAsset>("Data/Monster/GolemStatData").text).Mstat;
    ...
    private void LateUpdate()
    {
        if (Stat.Hp <= 0 && isDead == false)
        {
            ...
            GameManager.Sound.PlayEffectSound("Monster/Golem/GolemDeadSound");
            Invoke("DeadCall", 1.1f);
            _anim.Play("Die");
            isDead = true;
    ...
    {
        GameManager.Sound.PlayEffectSound("Monster/Golem/GolemAttackSound");
        ...
        Collider[] targetArr = Physics.OverlapSphere(transform.position, 2f, LayerMask.GetMask("Player"));
