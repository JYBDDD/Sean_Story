...
public class LichKingController : MonsterBaseController
{
    [SerializeField]
    ...
    public static LichKingController staticLichKing;  // IceMagic에서 스탯 불러오는것에 사용중

    public MonsterStat Stat { get => _stat; }

    private void OnEnable()
    {
        _stat = JsonConvert.DeserializeObject<StatData>(GameManager.Resource.Load<TextAsset>("Data/Monster/LichKingStatData").text).Mstat;

        isDead = false;
        staticLichKing = GetComponent<LichKingController>();
    ...
    private void LateUpdate()
    {
        if (Stat.Hp <= 0 && isDead == false)
        {
            ...
            _agent.SetDestination(transform.position + Random.onUnitSphere * 2);
            GameManager.Player.PlayerController.Stat.Exp += _stat.dropExp;
            GameManager.Sound.PlayEffectSound("Monster/Lich/LichKingDeadSound");
    ...
    private void AttackAnimEvent()  // 공격 애니메이션에 넣어줌
    {
        ...
        GameManager.Resource.Instantiate("UI/Particle_Effect/Monster/Lich/MonsterEffect/IceMagic_LichKing(Monster)", attackPoint.transform.position,
